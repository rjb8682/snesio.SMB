...
function fixController()
	...
	if controller["left"] then controller["P1 Left"] = true end
	if controller["right"] then controller["P1 Right"] = true end
	if controller["a"] then controller["P1 A"] = true end
	if controller["b"] then controller["P1 B"] = true end
end
...
function playGame(stateIndex, genome)
	...
	print("connecting")
	local player, err = socket.connect("127.0.0.1", 1080)
	print("connected")
	local serializedNet = binser.serialize(genome.network)
	player:send(#serializedNet .. "\n")
	player:send(serializedNet)
	...
	while true do
		...
		if currentFrame%4 == 0 then
			inputs = getInputs()
			dumpedInputs = binser.serialize(inputs)
			player:send(#dumpedInputs .. "\n")
			player:send(dumpedInputs)
			local move, err = player:receive()
			ok, controller = serpent.load(move)
			fixController()
		end
...
--local networkStr = loadNetworkFile("181404_13508766012.genomebinser")
local networkStr = nil
