...
loadConfigFile()
config.demoFile = "172714.3.genome"
config.drawGui = true
...
function playGame(stateIndex, genome)
	...
	while true do
		evaluateNetwork(network, getInputs())
...
function loadGenome(filename)
	...
	local ok, genome = serpent.load(file:read("*line"))
	file:close()
	return genome
end
...
if config.demoFile and config.demoFile ~= "" then
	...
	while true do
		...
		if z % 4 ~= 0 and z ~= 6 and z ~= 26 then
			maxFitness = maxFitness + calculateDemoFitness(playGame(z, loadGenome(config.demoFile)))
		end
