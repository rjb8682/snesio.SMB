...
Inputs = InputSize + 6 -- 3 feedback, marioVX, marioVY, BIAS NEURON
Outputs = #ButtonNames
...
function getInputs()
    ...
    -- Feedback
    inputs[#inputs+1] = hitLeftLastFrame
    inputs[#inputs+1] = hitRightLastFrame
    inputs[#inputs+1] = hitALastFrame
...
function setFeedback(button, value)
    if button == "left" then
        hitLeftLastFrame = value
    elseif button == "right" then
        hitRightLastFrame = value
...
function evaluateNetwork(network, inputs)
	...
	for o=1,Outputs do
		...
		if network.neurons[MaxNodes+o].value > 0 then
			outputs[button] = true
            setFeedback(button, 1)
		else
			outputs[button] = false
            setFeedback(button, 0)
		end
	end
